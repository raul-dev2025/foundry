...
REMOTE_BIN="${REMOTE_SRC_DIR}/pci_bar01"                # Binario específico de LTP
...
if ssh "${REMOTE_HOST}" "{ make -C ${REMOTE_SRC_DIR} clean && make -C ${REMOTE_SRC_DIR} pci_bar01; } > ${REMOTE_BUILD_LOG} 2>&1; cat ${REMOTE_BUILD_LOG}" > "${LOCAL_BUILD_LOG}" 2>&1; then
